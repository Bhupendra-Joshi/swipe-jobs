...
export class GetWorkerJobReject implements BaseApiHandler {
  /**
  ...
  async invoke(payload: JobActionRequestType): Promise<APIResult> {
    const result: any = await ApiService.getInstance()
      .get(API_PATH.WORKER_JOB_REJECT.replace('__WORKER_ID__', payload.workerId).replace('__JOB_ID__', payload.jobId))
